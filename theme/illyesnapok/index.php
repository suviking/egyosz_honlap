...
if (!isset($_GET["adminpage"]))		#dont wanted to use the admin site, goes directly to the registration form
{
	...
	if ($user["accessLevel"] == 3 OR $user["accessLevel"] == 2 OR $user["accessLevel"] == 1)	#if the user not a student but not an admin - redirect to adminpage
	{
...
#########
		...
		$result = $db->query("SELECT * FROM performances WHERE deleted = 0 ORDER BY id") OR die($db->error);
		...
		#########
			...
			echo("<table><tr>
				<td>id</td> <td>regStudID</td> <td>title</td> <td>category</td> <td>partNo</td> <td>location</td> <td>duration</td> <td>wiredMic</td> <td>wiredMicStand</td> 
				<td>wirelessMic</td> <td>wirelessMicStand</td> <td>microport</td> <td>fieldMic</td> <td>instMic</td> <td>chair</td> <td>musicFile</td> <td>projectorFile</td>
...
####checkas if the user has the access level to delete a performance
